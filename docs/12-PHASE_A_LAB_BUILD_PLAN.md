...
### 2.14 Actor rendering — Actor Forge contract (CORE-20) [P]

The lab consumes the verified Actor Forge v2.3 pipeline, not stand-in capsules: greybox fidelity applies to environment and effects, but actors ship real sheets because proving the asset pipeline is a stated lab goal.

- `addons/actorforge_importer`: imports the 32×32, 4-facing, 23-frame sheets; `tests/actor_sheet_slice` acceptance check verifies 4 facings × 23 frames × 32×32 per imported actor.
- View-layer `AnimatedActor` component (in `game/views/`): plays the sheet's animation sets; **facing derived view-side from sim velocity (movement) and aim (attacks)** — the sim knows nothing about facing or frames; animation state is presentation, driven by sim events (AttackStarted, HitLanded, EntityKilled) and never feeds back.
- `data/actor_sheet_map.tres` assigns the 12-actor roster: player shell + six roster enemies + Yard Warden (4 sheets spare).
- Milestones: player renders from its sheet at M2; the first two enemies at M5; the remaining roster + elite at M6. Acceptance lines in §4.
...
| Parameter | v0 | Tag |
|---|---|---|
| Tile size | 32 px | [P] (Actor Forge/TileForge contract, CORE-20) |
...
**M-FX — Effects pack authoring (art track, weeks 4–8, ~25–40 h).**
Designer authors the projectile shapes (shared hostile signature + per-family variants), telegraphs (rim/arm-indicator structure per §2.5), ground-hazard markers, hit/impact effects, and cast presentation against the nine acceptance rows (§ M6) — sequenced after M3 so real projectile motion exists to author against. This budget is subtracted from M3–M6 coding capacity. **Pre-registered decision, end of M5 (~week 7): pack or law-compliant placeholders for the tester build.**
*Accept:* pack delivered against the EffectLibrary interface, or the placeholder decision is recorded with a date.
...
Chaser + aimed shooter from EnemyDef.tres; 5-state machine; overhead HP bars (general presentation, no targeting — CORE-35); **both enemies rendering from Actor Forge sheets**; TTK event logging; density meter exercised with live hostiles; hostile-vs-friendly draw-order assertions active; **DodgeBot primary policy + both calibration canaries land here** (headless CLI minimal), so each EnemyDef ships with its proof in the milestone that authors it.
*Accept:* both behaviors killable without any target selection; deaths explainable via recap; both enemies render from Actor Forge sheets with correct facing from sim velocity/aim; **stress-density screenshot audit: hostile shots legible above maximum player VFX and above damage numbers** (Laws 1/2); Rusher + Husk carry passing 3.0 tiles/s ability-off proofs; canaries behave correctly (undodgeable fails, trivial passes).

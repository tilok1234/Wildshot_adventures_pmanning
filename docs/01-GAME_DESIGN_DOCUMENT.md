...
Wildshot Adventures is built by a solo developer who is highly proficient at orchestrating AI, with strong technical understanding and deep genre knowledge; production is AI-orchestrated across code, art, and content, with the developer as director, integrator, and designer. The engine is most likely Godot [P]. Abundant AI access removes tooling and asset-generation cost as a practical constraint; no contractor budget is assumed. Cash is not a project constraint (PROD-03, 2026-07-27): AI subscriptions are running and ample, the Steam fee is trivial, and music, SFX, and store art are all self-produced — the audio pipeline (in planning since 2026-07-27) inherits the forge rule and must encode readability Law 7 and the CORE-50 audio baseline from the start. A recorded sustainability note backs the no-deadline stance: capacity is financially backed by employment income, so there is no runway pressure; if life or income pressure ever imposes a de-facto deadline or forces capacity down, PROD-01 and the scope menu re-open by rule. The scope-menu decision (2026-07-27) is Option 4: the full game as specced remains the target, the floor-case ~2.25–4.5-year horizon is accepted in writing, and Gate 2's "viable full-game plan" is defined — the remaining bill at measured slice velocity must extrapolate to ≤ 5 years at the then-current PROD-01 floor, with the pre-registered cut order auto-triggering otherwise. Wildshot is the developer's primary project alongside a day job (PROD-01, 2026-07-27, amended the same evening): the stated project schedule is ~8 hours per weekday plus up to 16 per weekend day (≈72 h/week ceiling), and the reliable planning floor is 40 hours per week — explicitly a claim under test rather than a verified fact. Every plan-level figure divides by the floor, never the ceiling; the floor self-corrects against logged hours from Phase A onward, and no current decision depends on 40 holding (even a measured ~20 h/week lands the slice inside the accepted horizon). There is no deadline: the CORE-55 continuation gates, not dates, are the discipline mechanism.

The asset pipeline is verified shipping (showcase packet reviewed 2026-07-27). TileForge produces complete four-theme tile packages — 31,431 tiles across 80 families per theme, deterministic byte-identical regeneration, Godot importers, a pixel-match acceptance test, and interior coverage including dungeon-room, cave, and corrupt-zone scenes — with the quiet-floor readability discipline already encoded in generator output. Sprite Forge (2026-07-27, superseding Actor Forge v2.3) produces the actor and effects catalog: 231 deterministic, manifest-driven actors — 12 player armor kits, 128 enemies, 34 bosses including composite multi-part rigs, NPCs, vendors, props, and 20 projectile plus 15 effect sheets including telegraph rings, cones, and beams — on a 28-row humanoid rig at 64-pixel cells (32 logical × 2 scale), with per-actor recipes for deterministic regeneration and automatic theme remapping. Sheets are placeholder-fidelity by design: polished versions swap in later under the same ids and frozen contract. The pre-lab combat-effects task accordingly rescopes to curation and gap-fill — assigning player-versus-hostile projectile families, applying the shared hostile signature as a rendering treatment, and authoring hazard arm-progress indicators — still against the eight readability laws. A third tool, WorldForge (work in progress, completion committed), drafts whole zones from TileForge packages — settlement placement, tiered routes, rivers with fords and bridges, and a discoveries layer — under an explicit handcrafted-rule contract: generated worlds are drafts, the designer's curation pass makes them authored, and progression-critical placement is always hand-decided. Generated art must encode the readability laws from the start, generation tooling accelerates authoring without weakening the handcrafted-world rule, and production velocity remains a measured quantity through the lab and slice.
...
- detailed production planning and measured velocity (the hard constraints — solo, AI-orchestrated, Godot [P], a 40 h/week planning floor under self-correction (≈72 ceiling, PROD-01), no deadline, custom asset pipeline — are recorded in Section 15);
...
| CORE-20 | [L/P] | Section 15 — production constraints: solo, AI-orchestrated, Godot, 40 h/week floor under self-correction (PROD-01), no deadline |

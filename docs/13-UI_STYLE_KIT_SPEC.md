...
**2026-08-03 stamp:** the designer's MENU WORKBENCH shipped the v2 menu system (lock pin game←menu-workbench; delivery sl-0155 · intake sl-0158 · the pass sl-0159–0167). `menu-specs.json` is the LAYOUT SOURCE OF TRUTH for the v2 menus; the ornate look is DRAWN panel2 chrome (carved frame / gold studs / title plaque) generated from role-keyed palette tokens — `ui/menu_palette.gd` is THE ONE swap point. The v2 zip's 20 kit pieces + font proved BYTE-IDENTICAL to the consumed v1, so this 12×12 chrome contract stands untouched and re-skins now ride TOKENS + CAPTURES (the §4 palette-remap model, token-over-texture; four hues ship: dusk default / teal / slate / moss). §3's hold is further spent: quest journal, vendor, bank, options, toast/confirm all live on v2 surfaces (2026-08-03 menu pass); map screen, collection book, and the skill-tree screen remain future.
